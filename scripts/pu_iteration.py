...
#
parser.add_argument('--output_core', type=str, default='experiments/pu_liu_result',
...
def extract_reliable_negatives(model, data_stuff, unlabelled_idx, th=0.5, yshape=2, N_inputs=1):
   unlabelled_stuff = slice_data(data_stuff, unlabelled_idx, N_inputs)
   drugability = model.predict(unlabelled_stuff[0])
   if yshape==2:
      drugability=drugability[:,1]
   new_reliable_negatives_internal = np.where(drugability<th)[0]
   new_reliable_negatives = unlabelled_idx[new_reliable_negatives_internal]
...
while len(new_negatives)>0 and iteration<maxit:
   ...
   #update unlabelled
   new_negatives, stats, maybe_drug = extract_reliable_negatives(cv_results['model'], data_stuff, unlabelled_idx, th=args.th)
   unlabelled_idx = np.array([idx for idx in unlabelled_idx if idx not in new_negatives])
   ...
   logging.info('Reliable negatives: %i (new %i), unlabelled data: %i'%(N_neg, len(new_negatives), N_unlabelled))
